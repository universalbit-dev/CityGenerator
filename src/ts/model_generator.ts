...
import { STLExporter } from 'three/examples/jsm/exporters/STLExporter'; // Import STLExporter
...
export default class ModelGenerator {
    ...
    private readonly exporter = new STLExporter(); // Create an STLExporter instance
    ...
    public update(): boolean {
        switch (this.state) {
            ...
            case ModelGeneratorStates.SUBTRACT_OCEAN: {
                ...
                const seaLevelSTL = this.exporter.parse(seaLevelMesh); // Use STLExporter
                this.zip.file("model/domain.stl", seaLevelSTL);

                const seaMesh = this.polygonToMesh(this.sea, 0);
                this.threeToBlender(seaMesh);
                const seaMeshSTL = this.exporter.parse(seaMesh); // Use STLExporter
            ...
            case ModelGeneratorStates.ADD_COASTLINE: {
                ...
                const coastlineSTL = this.exporter.parse(coastlineMesh); // Use STLExporter
            ...
            case ModelGeneratorStates.SUBTRACT_RIVER: {
                ...
                const riverSTL = this.exporter.parse(riverMesh); // Use STLExporter
            ...
            case ModelGeneratorStates.ADD_ROADS: {
                if (this.polygonsToProcess.length === 0) {
                    ...
                    const roadsSTL = this.exporter.parse(mesh); // Use STLExporter
                    this.zip.file("model/roads.stl", roadsSTL);
            ...
            case ModelGeneratorStates.ADD_BLOCKS: {
                if (this.polygonsToProcess.length === 0) {
                    ...
                    this.threeToBlender(mesh);
                    const blocksSTL = this.exporter.parse(mesh); // Use STLExporter
                ...
                this.blocksGeometry = BufferGeometryUtils.mergeBufferGeometries([
                    this.blocksGeometry,
                    blockMesh.geometry as THREE.BufferGeometry,
                ]);
                break;
            }
            case ModelGeneratorStates.ADD_BUILDINGS: {
                if (this.buildingsToProcess.length === 0) {
                    ...
                    this.threeToBlender(mesh);
                    const buildingsSTL = this.exporter.parse(mesh); // Use STLExporter
